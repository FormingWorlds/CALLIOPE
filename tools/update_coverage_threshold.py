...
* fast  - updates `[tool.calliope.coverage_fast].fail_under` (unit/smoke gate)
...
def read_threshold_from_pyproject(target: str) -> float:
    ...
    try:
        ...
        if target == 'fast':
            return float(data['tool']['calliope']['coverage_fast']['fail_under'])
...
def update_threshold_in_pyproject(target: str, new_threshold: float) -> bool:
    ...
    if target == 'full':
        ...
        calliope_section = document.get('tool', {}).get('calliope')
        if calliope_section is None:
            raise ValueError('[tool.calliope] section not found in pyproject.toml')
        coverage_fast = calliope_section.get('coverage_fast')
        if coverage_fast is None:
            raise ValueError(
                '[tool.calliope.coverage_fast] section not found in pyproject.toml'
            )
