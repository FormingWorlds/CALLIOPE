#!/usr/bin/env python3
"""AST-based test-quality linter for the CALLIOPE test suite.
...
- Forbidden weak assertions when they stand alone as the sole meaningful
  ...
  three-class discrimination guard from calliope-tests.md section 2 uses
...
- Optional dependencies (``hypothesis``, ``atmodeller``) imported at module top
...
SRC_DIR = REPO_ROOT / 'src' / 'calliope'
...
# Source rule: calliope-tests.md section 5.
OPTIONAL_DEPS = {
    'hypothesis',
    'atmodeller',
}
...
# ``tests/test_<source>.py`` (the rule in calliope-tests.md section 12).
...
def cmd_baseline() -> int:
    ...
    allow = os.environ.get('CALLIOPE_TEST_QUALITY_ALLOW_REGRESS') == '1'
    if old and new_total > old_total and not allow:
        print(
            ...
            f'pre-existing violations), set CALLIOPE_TEST_QUALITY_ALLOW_REGRESS=1.',
            file=sys.stderr,
        )
        return 2
