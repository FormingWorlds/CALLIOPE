...
- **Solubility-law calibration.** The Dasgupta N$_2$ ($1373\le T \le 1873\,\mathrm{K}$) and Gaillard S$_2$ ($1473\le T \le 1973\,\mathrm{K}$) solubility laws have explicit $\ln f_{\mathrm{O}_2}$ terms. Outside their calibration range the solver still produces a solution, but the implied O partitioning between dissolved S/N and atmospheric SO$_2$/N$_2$ extrapolates the underlying experimental data. The entry point emits a `UserWarning` when $T_\mathrm{magma}$ falls outside $[1373, 1973]\,\mathrm{K}$.
