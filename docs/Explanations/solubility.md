...
where the prefactor $c_\mathrm{melt}$ depends on the silicate composition (CALLIOPE uses Earth-mantle estimates: $x_\mathrm{SiO_2} = 0.56$, $x_\mathrm{Al_2O_3} = 0.11$, $x_\mathrm{TiO_2} = 0.01$, giving $c_\mathrm{melt} = \exp(4.67 + 7.11 \cdot 0.56 - 13.06 \cdot 0.11 - 120.67 \cdot 0.01) \approx e^{6.01} \approx 408$). The two terms compete: at low $p_\mathrm{N_2}$ and reducing conditions, the first term (reduced N dissolved as nitride / amide complexes) dominates; at high $p_\mathrm{N_2}$ and oxidising conditions, the second term (molecular N$_2$ Henry's law) dominates. The breakpoint occurs roughly at $p_\mathrm{N_2}^{[\mathrm{GPa}]} \approx 10^{-5}$ for $\Delta\mathrm{IW} = 0$.
