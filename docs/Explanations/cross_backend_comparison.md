...
The figures here are regenerated by the reusable harness in `scripts/cross_backend/`; see [Reproducing this page](#reproducing-this-page) at the end.
...
Before comparing backends to each other, each must round-trip self-consistently: start from a buffered-mode call at a known $\Delta\mathrm{IW}$, take the resulting O budget, feed it back into the authoritative-O entry point, and recover the input $\Delta\mathrm{IW}$.

![Internal round-trip](../assets/figures/cross_backend/fig2_roundtrip.png)

*Figure 2. Round-trip residuals for each backend across the magma-ocean range $T \in \{1500, 2000, 2500, 3000\}\,\mathrm{K}$ at $\Delta\mathrm{IW} \in \{-2, 0, +2, +4\}$. Identity line shown in black. (a) CALLIOPE: residuals are below the solver's per-element tolerance ($10^{-3}$ dex) at every grid point. (b) atmodeller: residuals are similarly small, confirming that each backend is self-consistent across the redox range relevant to terrestrial magma oceans.*

A non-flat identity line in either panel would indicate a solver-precision problem in that backend, not a cross-backend disagreement. With this pre-flight check passing, the next figure can be read as honest backend-vs-backend physics, not solver noise.
...
Every figure on this page is regenerated by the harness in [`scripts/cross_backend/`](https://github.com/FormingWorlds/CALLIOPE/tree/main/scripts/cross_backend). To re-run:
...
The harness writes PDF and PNG outputs to `docs/assets/figures/cross_backend/` and raw CSV data to `scripts/cross_backend/data/`. Approximate total wall time on a modern Mac is twenty minutes, dominated by the atmodeller authoritative-O solver calls (~15 s warm, ~60 s cold first JAX compile per process).

The harness is reusable for different fiducials, different inventories, or different alignment configurations; see [`scripts/cross_backend/README.md`](https://github.com/FormingWorlds/CALLIOPE/blob/main/scripts/cross_backend/README.md) for the extension pattern.

## See also

- [Authoritative-oxygen mode](authoritative_oxygen.md): the entry-point contract shared by both backends.
