...
|---|---|---|---|
| IW buffer | Fischer et al. 2011[^cite-fischer2011] (default since the buffer audit, see below); O'Neill & Eggins 2002[^cite-oneilleggins2002] available as legacy `'oneill'` | Hirschmann composite (Hirschmann et al. 2008[^cite-hirschmann2008] below 1000 K, Hirschmann 2021[^cite-hirschmann2021] above) | Close (Fischer is within ~0.2 dex of Hirschmann across the magma-ocean range) |
...
## The buffer convention used to be the dominant systematic

CALLIOPE historically used the O'Neill & Eggins 2002 IW buffer; atmodeller uses the Hirschmann composite (Hirschmann 2008 below 1000 K, Hirschmann 2021 above). At magma-ocean temperatures these two parameterisations differ by up to $\sim 1$ dex, which was the single largest source of cross-backend $\Delta\mathrm{IW}$ disagreement. CALLIOPE also ships Fischer et al. 2011[^cite-fischer2011] as an alternative; the buffer audit that produced this page found that Fischer 2011 sits within $\sim 0.2$ dex of Hirschmann everywhere in the magma-ocean range, so we made it the new default. The legacy O'Neill choice remains available as `OxygenFugacity('oneill')` for backwards compatibility.
...
The buffer audit therefore takes a default residual of $\sim 1$ dex (with the legacy O'Neill setting) down to $\lesssim 0.2$ dex (with the new Fischer default). The remaining cross-backend disagreement is now genuinely chemistry-level (solubility laws, equilibrium-constant fits) rather than buffer-level.
