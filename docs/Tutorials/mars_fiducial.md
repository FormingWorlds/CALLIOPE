...
!!! note "On the choice of 'Mars-scaled' inventory"
    The Mars BSE H/C/N/S inventory is much less well-constrained than Earth's. To keep this tutorial reproducible without committing to a specific Mars-petrology estimate, we scale the Krijt+2023 Earth BSE inventory uniformly by Mars's mass-ratio to Earth ($0.107$). The result is a "Mars-mass terrestrial body with Earth-like volatile fractions". The contrast you read off the figure is therefore primarily driven by Mars's smaller gravity and mantle mass, not by a fundamentally different chemistry. For real Mars BSE work, swap in literature values (Wänke & Dreibus 1988, Taylor 2013, McSween et al. 2009) using the same workflow.
...
- Swap in literature Mars BSE values (Wänke & Dreibus 1988, Taylor 2013) in the `mars['hcns']` dict and re-run; compare the per-species shift to the uniform-scaling result.
